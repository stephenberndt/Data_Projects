    ...
    for adv_name, adv_id in config.settings.get_upload_batch:
        ...
        for workbook in config.settings.get_templates:
            ...
            if file_ext == '.twbx':
                ...
                utils.update_dates('output/uploaded_content/' + repointer.get_repointed_files()[0], datetime.datetime.now().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
                                   - relativedelta(months=1), datetime.datetime.now().replace(day=1, hour=0, minute=0, second=0, microsecond=0))
